...
export const ConfiguratorBar = ({ selectedOption, handleStandChange, handleColorChange, handleZoom, barActive, setBarActive, handleRimChange }) => {
    ...
    // // Reset mobile expanded state when active option changes
    // useEffect(() => {
    //     if (!activeOption) {
    ...
    useEffect(() => {
        selectedOption.label === "Sofa" ? setSelectedBar(SofaOptions) : selectedOption.label === "Fridge" ? setSelectedBar(RefrigeratorOptions) : setSelectedBar(Caroptions);
    }, [selectedOption]);
            ...
            <div className="fixed left-0 top-1/2 transform -translate-y-1/2 z-40">
                <button
                    ...
                    className={`
                        ...
                        ${barActive ? 'translate-x-[250px] md:translate-x-[280px]' : 'translate-x-0'}
                    `}
                    title={barActive ? "Hide Customize" : "Customize"}
                >
                    <FaChevronRight
                        className={`w-5 h-5 text-gray-700 transition-transform duration-300 ${barActive ? 'rotate-180' : ''}`}
                    />
                </button>
            </div>
                    ...
                    `}>
                        <div className="p-4 border-b border-gray-100">
                            <div className="flex items-center justify-between">
                                <h2 className="text-lg font-medium text-gray-800">Customize</h2>
                            </div>
                        </div>
                                                ...
                                                {option.subOptions?.map((subOption, subIndex) => (
                                                    ...
                                                    >
                                                        <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white shadow-md flex items-center justify-center bg-gray-50 hover:scale-110 transition-transform">
                                                            {subOption.iconUrl ? (
                                                                <img src={subOption.iconUrl} alt={subOption.label} className="w-full h-full object-cover" />
                                                            ) : (
                                                                <div className="text-base">{subOption.icon}</div>
                                                            )}
                                                        </div>
