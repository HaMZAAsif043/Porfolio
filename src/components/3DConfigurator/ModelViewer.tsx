...
// import { Fridge } from "./Fridge2.jsx";
...
const InteractiveModelViewer = () => {
    ...
    const colorHandlers = {
        Fridge: (color) => {
            ...
            if (FridgeRefs?.current?.current) FridgeRefs?.current?.current?.material?.color?.set(color);
            setBg(color)
        },
        Frame: (color) => {
            resetView()
        ...
        Seats: (color) => {
            ...
            if (CarRefs.current.leatherseatfr) CarRefs?.current?.leatherseatfr?.current.material?.color?.set(color);
            if (CarRefs.current.leatherseatrr) CarRefs?.current?.leatherseatrr?.current.material?.color?.set(color);
            ...
            if (CarRefs.current.steeringRef) CarRefs?.current?.steeringRef?.current.material?.color?.set("#1a1a1a");
            if (CarRefs.current.doorflref) CarRefs?.current?.doorflref?.current.material?.color?.set(color);
            if (CarRefs.current.doorrlRef) CarRefs?.current?.doorrlRef?.current.material?.color?.set(color);
            if (CarRefs.current.doorrrRef) CarRefs?.current?.doorrrRef?.current.material?.color?.set(color);
            if (CarRefs.current.doorfrRef) CarRefs?.current?.doorfrRef?.current.material?.color?.set(color);
            if (CarRefs.current.seatflRef) CarRefs?.current?.seatflRef?.current.material?.color?.set(color);
    ...
    return (
        <div className="flex flex-col md:flex-row h-full relative overflow-hidden">
            <div className="relative flex-1 w-full h-full">
                {/* Enhanced loader with fade transition */}
                {loading && (
                    <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center bg-black bg-opacity-60 z-50 transition-opacity duration-300 ease-in-out">
                        <div className="animate-spin rounded-full h-32 w-32 border-t-4 border-b-4 border-[#5ce1e6]"></div>
                        {/* <p className="text-white mt-4 font-medium text-lg">Loading {selectedOption.label}...</p> */}
                    </div>
                )}
                ...
                >
                    ...
                    <Suspense fallback={<LoadingIndicator />}>
                        ...
                        <group ref={ModelRef} position={[0, -1, 0]} rotation={[0, 0, 0]} scale={[1, 1, 1]}>
                            {{
                                "Fridge": <Fridge onRefsReady={handleRefsReady} ref={fridgeAnimation} castShadow />,
                                "Car": <BMW onRefsReady={handleRefsReady} ref={animationRef} rim={rim} castShadow />,
                                "Sofa": <Sofa onRefsReady={handleRefsReady} standType={standType} castShadow />,
                ...
                <div className="fixed top-10 lg:top-auto lg:bottom-6 left-1/2 transform -translate-x-1/2 z-40">
                    <button
                        ...
                        className="bg-[#359dad] px-4 py-2 rounded-full shadow-lg hover:bg-[#2c8c9a] transition-colors flex items-center justify-center gap-2"
                        title="View in Space"
                    >
                        ...
                        <span className="text-sm font-medium text-white sm:inline">View in Space</span>
                    </button>
                </div>
                        ...
                        {selectedOption.currentModel.controls?.map((control) => (
                            <button
                                ...
                                className={`relative bg-white p-3 rounded-full shadow-lg hover:bg-gray-100 transition-all duration-200 
                                  ${activeControl === control.action ? 'bg-gray-100' : ''}
                                `}
                                title={control.label}
                            ...
                            >
                                ...
                                {animLoading && activeControl === control.action && (
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-[#359dad]"></div>
                                    </div>
                                )}

                                {/* Icon with opacity controlled by loading state */}
                                <div className={animLoading && activeControl === control.action ? 'opacity-0' : 'opacity-100'}>
                ...
                {selectedOption.label === "Fridge" && (
                    <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex flex-col gap-3 z-40">
                        <button
                            ...
                            className={`relative bg-white p-3 rounded-full shadow-lg hover:bg-gray-100 transition-all duration-200 
                              ${activeControl === "Left_DoorAction" ? 'bg-gray-100' : ''}
                            `}
                        ...
                        >
                            ...
                            <div className={animLoading && activeControl === "Left_DoorAction" ? 'opacity-0' : 'opacity-100'}>
                        ...
                        <button
                            ...
                            className={`relative bg-white p-3 rounded-full shadow-lg hover:bg-gray-100 transition-all duration-200 
                              ${activeControl === "Right_DoorAction" ? 'bg-gray-100' : ''}
                            `}
                        ...
                        >
                            ...
                            <div className={animLoading && activeControl === "Right_DoorAction" ? 'opacity-0' : 'opacity-100'}>
                                {renderControlIcon("Right_DoorAction", "Fridge")}
                            </div>
                        </button>
                        ...
                        <button
                            ...
                            className={`relative bg-white p-3 rounded-full shadow-lg hover:bg-gray-100 transition-all duration-200 
                              ${activeControl === "RefrigeratorAction" ? 'bg-gray-100' : ''}
                            `}
                        ...
                        >
                            ...
                            <div className={animLoading && activeControl === "RefrigeratorAction" ? 'opacity-0' : 'opacity-100'}>
    ...
    pb-14 md:pb-0 ${showDetails ? "translate-y-0" : ""} z-50
`}>
                <div className="h-2 w-16 bg-gray-300 rounded-full mx-auto my-2 md:hidden" onClick={() => setShowDetails(!showDetails)}></div>
                <ConfigSidebar
                    Options={Options}
