...
const ConfigSidebar = ({
  ...
  showDetails
        ...
        <div className="flex items-center mb-1">
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground">
              {selectedOption.currentModel.label}
            </h3>
          ...
          {Object.entries(selectedOption.currentModel.details)
            ...
            .map(([key, value]) => (
              <li key={key} className="flex flex-col">
                <h4 className="text-sm font-bold text-primary capitalize mb-1">
                  {key.replace(/([A-Z])/g, " $1")}:
                </h4>
    ...
    const currentIndex =
      Array.isArray(Options) && Options.length > 0 && selectedOption
    ...
    return (
      <div
        className={`
            ...
            ${
              isExpanded
                ? "h-auto max-h-full overflow-auto"
                : "h-auto max-h-[75vh] md:max-h-full"
            }
            transition-all duration-300
        `}
      >
        {/* Model selector with navigation arrows */}
        <div className="relative bg-background pb-3">
          {/* Left navigation arrow */}
          <button
            onClick={goToPrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-muted rounded-full p-2 shadow-md"
          ...
          <button
            onClick={goToNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-muted rounded-full p-2 shadow-md"
            aria-label="Next model"
            disabled={
              !Array.isArray(Options) || currentIndex === Options.length - 1
            }
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
